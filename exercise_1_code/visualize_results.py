...
class FitnessLandScapeCanvas2D(FitnessLandScapeCanvas):
    def produce_scatter(self):
        #
        x = self.individuals_data[0::2]
        y =  self.individuals_data[1::2]

        self.scatter = self.axes.scatter(x, y, c='r', marker='o', zorder = 4, s=50)
        self.axes.hold(True)

        # self.axes.set_ylim([0.5, 2.5])

        best_individual = np.argmax(y)
        self.scatter_best = self.axes.scatter(x[best_individual], y[best_individual], c='g', marker='o', zorder = 4, s=120)
...
class FitnessLandScapeCanvas3D(FitnessLandScapeCanvas):
    def produce_scatter(self):
        ...
        #unpack is incorrect for single value
        if self.individuals_data.size == 3:
            self.individuals_data = [self.individuals_data]

        x = [d[0] for d in self.individuals_data]
        y = [d[1] for d in self.individuals_data]
        z = [d[2] for d in self.individuals_data]
